...
export default function ContractsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return <Badge className="bg-amber-50 border border-amber-100 text-amber-700 text-[10px] font-extrabold uppercase px-2 py-0.5 rounded">En attente</Badge>
      case 'signed':
        return <Badge className="bg-blue-50 border border-blue-100 text-blue-700 text-[10px] font-extrabold uppercase px-2 py-0.5 rounded">Signé</Badge>
      case 'completed':
        return <Badge className="bg-emerald-50 border border-emerald-100 text-emerald-700 text-[10px] font-extrabold uppercase px-2 py-0.5 rounded">Finalisé</Badge>
      case 'cancelled':
    ...
    <div className="p-6 max-w-7xl mx-auto pb-24 space-y-8 animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1.5 text-sm font-light leading-relaxed">
            Consultez, signez numériquement vos contrats et téléchargez les copies certifiées.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 bg-slate-100/50 p-1.5 rounded-2xl w-fit border border-slate-200/50">
        {(['all', 'pending', 'signed', 'completed', 'cancelled'] as const).map((tab) => (
          ...
          >
            ...
            {tab === 'signed' && 'Signés'}
            {tab === 'completed' && 'Finalisés'}
            {tab === 'cancelled' && 'Annulés'}
          </button>
        ))}
      </div>
      ...
      {isLoading ? (
        ...
        <Card className="p-16 bg-white border border-dashed rounded-[2.5rem] text-center border-gray-200">
          ...
          <h3 className="text-sm font-bold text-gray-800">Aucun document trouvé</h3>
          <p className="text-xs text-gray-400 mt-2 max-w-sm mx-auto leading-relaxed font-light">
            Vous n'avez pas de contrats correspondant à cette catégorie. Les contrats se créent automatiquement pour les services et événements payants.
          </p>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredContracts.map((contract) => {
            ...
            const roleLabel = isClient ? 'Bénéficiaire' : 'Prestataire/Créateur'
                ...
                <div className="p-6 space-y-4">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-[10px] text-slate-400 font-medium flex items-center gap-1">
                      <Calendar size={12} />
                      {format(new Date(contract.createdAt), 'dd MMMM yyyy', { locale: fr })}
                    </span>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500 font-light flex items-center gap-1">
                      Rôle : <strong className="font-semibold text-[#0c3383]">{roleLabel}</strong>
                    </p>
                  </div>
                  ...
                  {otherParty && (
                    <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100 text-xs shrink-0">
                      ...
                      <div className="truncate">
                        <p className="text-slate-400 font-light text-[9px] uppercase tracking-wider">Avec le voisin</p>
                  ...
                  <div className="flex justify-between items-center text-xs border-t border-slate-50 pt-3">
                    <span className="text-slate-400 font-light">Valeur de l'échange :</span>
                    <span className="font-extrabold text-[#0c3383] text-sm">🪙 {contract.pricePoints} pts</span>
                  </div>
                </div>
      ...
      <div className="bg-emerald-50/40 border border-emerald-100 rounded-3xl p-5 flex items-start gap-3 max-w-2xl text-xs text-emerald-800 leading-relaxed font-light">
        ...
        <div>
          <strong className="font-bold">Sécurisation et RGPD :</strong> Vos signatures électroniques sont chiffrées et certifiées par la plateforme Hoodly. Dans le cadre du RGPD, les données d'audit associées (adresse IP, signatures) sont protégées et archivées de manière sécurisée. Vous pouvez exporter vos données personnelles depuis votre espace profil.
        </div>
      </div>
    </div>
  )
